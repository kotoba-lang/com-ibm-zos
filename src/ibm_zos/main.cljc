(ns ibm-zos.main
  "Kotodama WASM entrypoint for the IBM z/OS clean-room actor (L4) — Clojure port.
  ...
  facts (`ibm_zos.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ibm_zos")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ibm_zos.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ibm_zos-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ibm_zos-compat/ready)
